...
class UploadEvent(PatternMatchingEventHandler):
  ...
  #             print("key{}, val{}:".format(x, data_dict[x]))
       ...
       # path = path
        ...
        startDate = datetime.fromtimestamp(myTimestamp, tz=pytz.UTC).strftime('%Y-%m-%dT%H:%M')
        print("Start date:" + startDate)
        myTimestamp = endDate / dataRate
        endDate =   datetime.fromtimestamp(myTimestamp, tz=pytz.UTC).strftime('%Y-%m-%dT%H:%M')
        print("End date:"+endDate)
        command = "python3 psws_addOBS.py " + str(dataRate) + " " + str(obsSize) + " "  \
                  + fileName + " " + datapath + " " + station_id + " " + startDate  \
                  + " " + endDate + " "
