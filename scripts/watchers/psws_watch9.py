        ...
        if event.is_directory:
            ...
            if leaf in TRIGGER_NAMES or leaf[0] in TRIGGER_NAMES:
                ...
                if (
                    ...
                    # Trigger graphing for each magnetometer file
                    try:
                        writeLog("Trigger magnetometer graphing program")
                        import glob
                        import re

                        # Get station info for plotting
                        # Import Django models to get station metadata
                        import django
                        ...
                        if not station_obj:
                            writeLog(
                                "ERROR: Station "
                                + station_id
                                + " not found for plotting."
                            )
                        else:
                            # Gather candidates (zip, csv, json files)
                            candidates = (
                            ...
                            for fpath in sorted(candidates):
                                date_str = None
                                m = re.search(
                                    r"(\d{4}-\d{2}-\d{2})", os.path.basename(fpath)
                                )
